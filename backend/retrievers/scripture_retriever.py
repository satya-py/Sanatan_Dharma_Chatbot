...
class ScriptureRetriever:
    ...
    def load_vector_store(self):
        if os.path.exists(self.db_path) and len(os.listdir(self.db_path)) > 0:
            print(f"[ScriptureRetriever] Loading existing scripture FAISS index from {self.db_path}")
            ...
            self.vector_retriever = self.vector_store.as_retriever(search_kwargs={"k": 8})
        else:
            raise FileNotFoundError(
                f"Scripture FAISS index not found at {self.db_path}. Please ensure it is present."
            )

    def load_bm25_retriever(self):
        print("[ScriptureRetriever] Initializing BM25 corpus from cached chunks and Mahabharata JSON...")
        ...
        if os.path.exists(settings.MAHABHARATA_JSON_PATH):
            ...
            print(f"[ScriptureRetriever] Loaded {len(mahabharata_chunks)} Mahabharata chunks.")
        else:
            print(f"[ScriptureRetriever] Warning: Mahabharata progress JSON not found at {settings.MAHABHARATA_JSON_PATH}")
            
        # Compile texts and metadatas
        texts = [c["text"] for c in pdf_chunks] + [c["text"] for c in mahabharata_chunks]
        metadatas = (
            [{"source": c["source"], "reference": c["reference"]} for c in pdf_chunks] +
            [{"source": c["source"], "reference": c["reference"], "book_num": c.get("book_num")} for c in mahabharata_chunks]
        )
        ...
        print(f"[ScriptureRetriever] BM25 corpus initialized with {len(bm25_docs)} documents.")

    def setup_ensemble_retriever(self):
        if self.bm25_retriever and self.vector_retriever:
            print("[ScriptureRetriever] Initializing Ensemble Retriever (weights: 0.4 BM25, 0.6 FAISS)...")
    ...
    def retrieve(self, query: str, llm=None, use_multiquery: bool = True, top_k: int = 5):
        ...
        if use_multiquery and MultiQueryRetriever and llm:
        ...
        reranked_docs = embedding_manager.rerank(query, raw_docs, top_k=top_k)
